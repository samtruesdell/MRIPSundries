...
restrictSize <- function(sDat, open, group = NULL, common, fun){

  # Check to see whether each year in the data is longer than
  # dateStart to dateEnd

  if(class(open) == 'character'){
    if(length(open) != 2){
  ...
  }else if(class(open) == 'list'){
    lenCheck <- sapply(open, length)
    if(!all(lenCheck == 2)){
  ...
  dat <- mutate(dat, DATE = get_date(dat = dat))
  ...
  openLst <- lapply(open, function(x) data.frame(YEAR = yrs,
                                                      st = x[1],
                                                      nd = x[2]))
  openDF <- do.call(rbind, openLst)

  if(FALSE){
    openDF$st <- with(openDF, as.Date(paste(YEAR, st, sep='-')))
    openDF$nd <- with(openDF, as.Date(paste(YEAR, nd, sep='-')))
  }
  ...
  uCom <- unique(dat$COMMON)
  plotList <- list()
  for(i in seq_along(uCom)){
    p <- dat %>%
      filter(COMMON == uCom[i]) %>%
      group_by(YEAR) %>%
      ggplot(aes_string(val)) +
      ...
      geom_rect(data = openDF,
                ...
                ymin=-Inf, ymax = 0,
                inherit.aes = FALSE) +
      scale_fill_manual('Legal harvest',
